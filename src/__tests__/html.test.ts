import { $event, $props } from "../attribute";
import { ATTRIBUTE_EVENT_NAME, ATTRIBUTE_PROPS_NAME, html, htmlToString, isCustomElement } from "../html";
...
describe('htmlToString()', () => {
  ...
  it('should convert TemplateResult that has event to html string', () => {
    ...
    expect(htmlToString(template)).toBe(`<div><button data-${ATTRIBUTE_EVENT_NAME}="true">submit</button></div>`);
  });
  ...
  it('should convert TemplateResult that has props to html string', () => {
    ...
    expect(htmlToString(template)).toBe(`<custom-element data-${ATTRIBUTE_PROPS_NAME}="true"><template shadowroot="open"><h1>Hello World</h1></template></custom-element>`);
  });
  ...
  it('should convert TemplateResult that has TemplateResult to html string', () => {
    ...
    expect(htmlToString(template)).toBe(`<custom-element><template shadowroot="open"><div><h1>Hello World</h1>${expectStringTemplateChild}</div></template></custom-element>`);
  });
});
